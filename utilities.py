...
def traktJsonRequest(method, req, args={}, returnStatus=False, anon=False, conn=False, silent=True, passVersions=False):
    ...
    try:
        ...
        if method == 'POST':
            ...
            return None
        Debug("traktQuery: "+method+" JSON url: "+req)
    except socket.error:
    ...
    try:
        #~ Debug("traktQuery: JSON response: "+raw)
        data = json.loads(raw)
        Debug("traktQuery: JSON response: "+str(data))
    except ValueError:
...
def rateEpisodeOnTrakt(tvdbid, title, year, season, episode, rating):
    ...
    Debug("[Rating] Rating episode '"+title+"' "+str(season)+"x"+str(episode)+" :" + rating)

    response = traktJsonRequest('POST', '/rate/episode/%%API_KEY%%', {'tvdb_id': tvdbid, 'title': title, 'year': year, 'season': season, 'episode': episode, 'rating': rating}, passVersions=True)
    if response == None:
        Debug("Error in request from 'rateEpisodeOnTrakt()'")
        return response

    if (rating == "unrate"):
        notification(__language__(1201).encode( "utf-8", "ignore" ), __language__(1166).encode( "utf-8", "ignore" )) # Rating removed successfully
    else :
...
def getEpisodeRatingFromTrakt(tvdbid, title, year, season, episode, ratingType):
    if tvdbid == "" or tvdbid == None:
        return None #would be nice to be smarter in this situation

    #~ Debug("Calling getEpisodeRatingFromTrakt()")
    response = traktJsonRequest('GET', '/show/episode/summary.json/%%API_KEY%%/'+str(tvdbid)+"/"+str(season)+"/"+str(episode))
    if response == None:
        Debug("Error in request from 'getEpisodeRatingFromTrakt()'")
        return None
    #~ Debug("getEpisodeRatingFromTrakt() result="+str(response))

    if 'episode' in response:
        if ratingType == "advanced" and 'rating_advanced' in response['episode']:
            return response['episode']['rating_advanced']
        elif 'rating' in response['episode']:
            return response['episode']['rating']
...
def rateShowOnTrakt(tvdbid, title, year, rating):
    ...
    Debug("[Rating] Rating show '"+title+"':" + rating)

    response = traktJsonRequest('POST', '/rate/show/%%API_KEY%%', {'tvdb_id': tvdbid, 'title': title, 'year': year, 'rating': rating}, passVersions=True)
    if response == None:
...
def getShowRatingFromTrakt(tvdbid, title, year, ratingType):
    if tvdbid == "" or tvdbid == None:
        return None #would be nice to be smarter in this situation

    response = traktJsonRequest('GET', '/show/summary.json/%%API_KEY%%/'+str(tvdbid))
    if response == None:
        Debug("Error in request from 'getShowRatingFromTrakt()'")
        return None

    if ratingType == "advanced" and 'rating_advanced' in response:
        return response['rating_advanced']
...
def rateMovieOnTrakt(imdbid, title, year, rating):
    ...
    Debug("[Rating] Rating movie '"+title+"':" + rating)

    data = traktJsonRequest('POST', '/rate/movie/%%API_KEY%%', {'imdb_id': imdbid, 'title': title, 'year': year, 'rating': rating}, passVersions=True)
    if data == None:
...
def getMovieRatingFromTrakt(imdbid, title, year, ratingType):
    if imdbid == "" or imdbid == None:
        return None #would be nice to be smarter in this situation

    response = traktJsonRequest('GET', '/movie/summary.json/%%API_KEY%%/'+str(imdbid))
    if response == None:
